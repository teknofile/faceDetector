 ...
 */
class faceDetector_maintain extends PluginMaintain
{
  ...
  function __construct($plugin_id)
  {
    ...
    $this->table = $prefixeTable . 'faceDetector';
    $this->dir = PHPWG_ROOT_PATH . PWG_LOCAL_DIR . 'faceDetector/';
  }
  ...
  function install($plugin_version, &$errors=array())
  {
    ...
    if (empty($conf['faceDetector']))
    {
      ...
      conf_update_param('faceDetector', $this->default_conf, true);
    }
    else
    {
      $old_conf = safe_unserialize($conf['faceDetector']);
      ...
      conf_update_param('faceDetector', $old_conf, true);
    }
...
;');
    ...
    $result = pwg_query('SHOW COLUMNS FROM `'.IMAGES_TABLE.'` LIKE "faceDetector";');
    if (!pwg_db_num_rows($result))
    {
      pwg_query('ALTER TABLE `' . IMAGES_TABLE . '` ADD `faceDetector` TINYINT(1) NOT NULL DEFAULT 0;');
    }
  ...
  function uninstall()
  {
    ...
    conf_delete_param('faceDetector');

    // delete table
    pwg_query('DROP TABLE `'. $this->table .'`;');

    // delete field
    pwg_query('ALTER TABLE `'. IMAGES_TABLE .'` DROP `faceDetector`;');
