...
<fieldset>
  ...
  <ul>
    <li>
      <label>
        <input type="checkbox" name="option2" value="1" {if $faceDetector.option2}checked="checked"{/if}>
        <b>{'Checkbox'|translate}</b>
      </label>
      <a class="icon-info-circled-1 showInfo" title="{'Check me!'|translate}"></a>
    </li>
    <li class="option1" {if not $faceDetector.option2}style="display:none;"{/if}>
      <label>
        <b>{'Integer'|translate}</b>
        <input type="text" name="option1" value="{$faceDetector.option1}" size="4">
      </label>
    </li>
    <li>
      <label>
        <b>{'Select'|translate}</b>
        {html_options name=option3 options=$select_options selected=$faceDetector.option3}
      </label>
    </li>
  </ul>
</fieldset>
